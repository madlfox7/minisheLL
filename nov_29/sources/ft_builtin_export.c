...
int ft_export(char **str, t_environment_list **envp, int exit_num)
{
    ...
    while (str[i])     
    {
        ...
        if ( !ft_wrong_name(name, str[i]))  //CHECKING 
        {
...
// t_environment_list *ft_list_new_for_export(char *string_from_envp, char *backup_name)
// {
//     (void)backup_name;
...
//     else if (!ft_strchr(string_from_envp, '='))
//         index = -42;
//     else
//         index = get_index_of_char(string_from_envp, '=');
...
result->name_and_value = (char **)malloc(2 * sizeof(char *));
    ...
    result->name_and_value[0] = strdup(name); 
    if (value) 
    {
        result->name_and_value[1] = strdup(value); 
