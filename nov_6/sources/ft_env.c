...
void print_env(t_environment_list *env)
{
    ...
    while (current != NULL)
    {
        if (current->env_flag == 1) //printing in env only if it has name and value
        {
            printf("%s=%s\n", current->name_and_value[0], current->name_and_value[1]);  //if has assigned value not only ARG
        }
        current = current->next;
    }
}